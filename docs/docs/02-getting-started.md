...
| `TRUST_FORWARDED_HEADERS`            | ❌       | Trust client-supplied forwarded headers (`X-Mcp-Oauth-Proxy-URL`, `X-Forwarded-Proto`, `X-Forwarded-For`, `X-Real-IP`). Default `true`. Governs (1) the external base URL — redirect URIs, metadata, `WWW-Authenticate` — from `X-Mcp-Oauth-Proxy-URL` / `X-Forwarded-Proto`; (2) the per-IP rate-limit client IP for `/authorize`, `/token`, `/register` from `X-Forwarded-For` / `X-Real-IP`; and (3) the automatic cookie `Secure` decision (`COOKIE_SECURE=auto`) from `X-Forwarded-Proto`. Set `false` when the proxy is **not** behind a trusted reverse proxy; the scheme is then derived from the connection (TLS), the host from the request `Host`, and the rate-limit client IP from `RemoteAddr`, so a client cannot spoof these headers to rotate the rate-limit key or flip the cookie `Secure` flag. `Host` should itself be constrained at the infrastructure layer |
